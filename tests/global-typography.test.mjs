...
assert.match(readFileSync(new URL('../src/app.mjs', import.meta.url), 'utf8'), /<details class="builder-market-switcher"[\s\S]{0,220}Change market/, 'Builders should expose a clear market-change action via progressive disclosure before secondary evidence');
...
assert.match(appSource, /<span class="eyebrow">Builders · selected market<\/span>/, 'Builders header should enter through the selected market, not a poster-scale state chooser');
assert.match(appSource, /<details class="builder-market-switcher"[\s\S]{0,220}Change market/, 'Builders action copy should stay short and calm behind a market-change disclosure');
