...
const builderMarketSources = [
  ...
  { key: 'lafayette-in', state: 'IN', marketName: 'Lafayette / Tippecanoe County, IN', csvUrl: 'data/real/lafayette-in/builder_validation_queue.csv', signalsUrl: 'data/real/lafayette-in/builder_signals.json', evidenceUrl: 'data/real/lafayette-in/market_evidence.json' },
];
...
const builderMarketRegistry = [
  ...
  { key: 'rocky-mount-nc', state: 'NC', label: 'Rocky Mount / Nash-Edgecombe', note: 'SF development-site buyer-validation lane', platform: 'Public builder/community signals; city CSS permit search gated', suggestedRank: 14, sourceWork: 'Call top buyer-builders to capture buy box before seller sourcing.' },
  ...
  { key: 'lafayette-in', state: 'IN', label: 'Lafayette / Tippecanoe County', note: 'official Indiana owner/developer permit lane', platform: 'Tippecanoe County GIS BuildingPermits_Adr ArcGIS', suggestedRank: 30.5, sourceWork: 'Enrich owner/developer source rows to company/contact/buy-box before any seller sourcing.' },
...
function renderLandControls() {
  if (selectedDealsMarketKey === 'all') return renderLandMarketIndex();
  const parcels = scoredParcels();
  const activeMarket = getSelectedDealsMarket();
  const visibleRows = parcels.filter(parcel => {
  ...
  const listingStates = visibleRows.map(parcelListingState);
  ...
  const selectedState = selectedLandStateFilter === 'all' ? (activeMarket?.stateCode || activeMarket?.state || 'Market') : selectedLandStateFilter;
  const sortOptions = [
    ['priority', 'Priority'],
    ['state', 'State'],
    ['market', 'Market'],
  ...
  const sortControl = `<div class="land-control-group sort land-refined-sort" aria-label="Sort land listings"><span>Sort</span><div>${sortButtons}</div></div>`;
  const selectedMarketLabel = activeMarket ? (activeMarket.marketLabel || activeMarket.label) : 'Selected market';
...
function renderLandMarketIndex() {
  ...
  const allMarketEntries = dealsMarketCoverageEntries().filter(market => market.key !== 'all');
  const stateMarketSummaries = stateOptions.filter(state => state !== 'all').map((state) => {
    ...
    return {
      ...
      tone: stateParcels.length ? 'is-live' : stateMarkets.some(market => Number(market.builderCount || 0) > 0) ? 'is-thin' : 'needs-work',
    };
  });
  const marketCount = selectedLandStateFilter === 'all' ? stateMarketSummaries.length : entries.length;
  const liveSellerCount = selectedLandStateFilter === 'all'
    ? stateMarketSummaries.reduce((sum, state) => sum + Number(state.dealCount || 0), 0)
    : entries.reduce((sum, market) => sum + Number(market.dealCount || 0), 0);
  const liveBuilderCount = selectedLandStateFilter === 'all'
    ? stateMarketSummaries.reduce((sum, state) => sum + Number(state.builderCount || 0), 0)
    : entries.reduce((sum, market) => sum + Number(market.builderCount || 0), 0);
  const stateFilters = stateOptions.map(state => {
    const isActive = selectedLandStateFilter === state;
...
function renderDealsMarketCoverage() {
  ...
  const stateButtons = getLandStateOptions().map(state => {
    const isActive = selectedLandStateFilter === state;
    const label = state === 'all' ? 'All' : state;
    return `<div role="button" tabindex="0" class="land-index-state-filter land-command-state ${isActive ? 'active' : ''}" data-land-state="${h(state)}" aria-pressed="${isActive ? 'true' : 'false'}">${h(label)}</div>`;
  }).join('');
  ...
  return `<section class="deals-market-coverage land-market-lane-selector land-market-command-rail phase254-land-market-command-rail phase255-state-gated-market-rail phase256-selected-market-nav" aria-label="Land market command rail">
    <div class="deals-market-head land-market-command-head">
      ...
      <div role="button" tabindex="0" class="land-back-to-state-index" data-land-state="all" aria-label="Back to main Land state selection">← State index</div>
    </div>
    <div class="land-selected-state-switcher" aria-label="Change Land state while viewing a market">${stateButtons}</div>
    <div class="deals-market-grid land-market-command-scroll" role="listbox" aria-label="Select Land market inside selected state">${marketButtons}</div>
  </section>`;
}
...
const BUILDER_STATE_SILHOUETTES = Object.freeze({
  FL: { viewBox: '-87.9 24.25 8.13 7.02', path: 'M-80.75 24.86L-80.7 24.91L-80.68 24.88L-80.73 24.84L-80.75 24.86Z M-80.85 24.8L-80.82 24.84L-80.78 24.84L-80.8 24.81L-80.85 24.8Z M-81.12 24.71L-80.94 24.78L-80.92 24.76L-81.08 24.69L-81.12 24.71Z M-81.81 24.57L-81.74 24.66L-81.67 24.7L-81.58 24.74L-81.57 24.76L-81.44 24.81L-81.31 24.76L-81.29 24.71L-81.24 24.67L-81.26 24.66L-81.4 24.62L-81.41 24.65L-81.51 24.64L-81.52 24.62L-81.68 24.56L-81.81 24.55L-81.81 24.57Z M-82.01 24.54L-81.87 24.56L-81.91 24.53L-81.97 24.51L-82.02 24.52L-82.01 24.54Z M-82.16 24.56L-82.14 24.59L-82.1 24.58L-82.12 24.55L-82.16 24.56Z M-82.27 26.7L-82.25 26.68L-82.18 26.5L-82.15 26.48L-82.09 26.46L-82.06 26.47L-82.01 26.45L-82.06 26.43L-82.1 26.42L-82.17 26.47L-82.2 26.49L-82.24 26.59L-82.27 26.66L-82.27 26.7Z M-82.93 24.63L-82.88 24.64L-82.86 24.62L-82.94 24.62L-82.93 24.63Z M-84.67 29.79L-84.57 29.84L-84.57 29.81L-84.62 29.78L-84.66 29.77L-84.67 29.79Z M-85 31L-84.98 30.93L-84.94 30.89L-84.94 30.82L-84.91 30.75L-84.86 30.71L-84.38 30.69L-83.44 30.64L-82.7 30.6L-82.21 30.57L-82.23 30.56L-82.23 30.51L-82.2 30.49L-82.21 30.42L-82.17 30.36L-82.04 30.37L-82.04 30.43L-82.02 30.48L-82.01 30.58L-82.05 30.66L-82.04 30.71L-82.04 30.75L-82.01 30.76L-82.02 30.78L-81.98 30.78L-81.95 30.83L-81.9 30.82L-81.87 30.79L-81.81 30.79L-81.74 30.77L-81.72 30.74L-81.6 30.73L-81.54 30.71L-81.49 30.73L-81.44 30.71L-81.43 30.7L-81.44 30.6L-81.43 30.52L-81.45 30.5L-81.41 30.48L-81.4 30.4L-81.39 30.3L-81.31 29.97L-81.26 29.86L-81.26 29.81L-81.16 29.56L-81.05 29.31L-80.94 29.11L-80.71 28.76L-80.57 28.59L-80.56 28.53L-80.52 28.46L-80.59 28.41L-80.6 28.36L-80.61 28.29L-80.59 28.18L-80.57 28.1L-80.51 27.97L-80.38 27.74L-80.33 27.6L-80.31 27.53L-80.29 27.5L-80.26 27.38L-80.19 27.25L-80.16 27.19L-80.14 27.11L-80.09 27.02L-80.03 26.8L-80.04 26.59L-80.06 26.44L-80.08 26.26L-80.09 26.23L-80.12 25.99L-80.12 25.84L-80.13 25.77L-80.15 25.7L-80.15 25.67L-80.18 25.68
L-113.69 45.28L-113.69 45.25L-113.6 45.19L-113.57 45.13L-113.51 45.12L-113.52 45.09L-113.45 45.06L-113.44 45.01L-113.45 44.96L-113.5 44.95L-113.46 44.87L-113.36 44.82L-113.34 44.79L-113.25 44.82L-113.13 44.77L-113.07 44.68L-113.05 44.64L-113.08 44.6L-113.01 44.53L-113.03 44.5L-113 44.45L-112.95 44.42L-112.89 44.4L-112.85 44.36L-112.81 44.38L-112.84 44.42L-112.78 44.48L-112.72 44.5L-112.66 44.49L-112.54 44.48L-112.5 44.46L-112.47 44.48L-112.39 44.45L-112.36 44.49L-112.35 44.54L-112.32 44.54L-112.29 44.57L-112.23 44.56L-112.19 44.53L-112.14 44.54L-112.11 44.52L-112.03 44.55L-112 44.54L-111.95 44.56L-111.87 44.56L-111.82 44.51L-111.72 44.54L-111.7 44.56L-111.62 44.55L-111.59 44.56L-111.47 44.54L-111.53 44.6L-111.47 44.68L-111.49 44.71L-111.41 44.71L-111.39 44.76L-111.32 44.72L-111.22 44.62L-111.23 44.59L-111.14 44.54L-111.12 44.49L-111.05 44.47L-111.05 44.35L-111.05 44.12L-111.05 43.68L-111.04 43.66L-111.04 42.72L-111.05 42.58L-111.05 42L-111.47 42L-112.17 42L-112.19 42L-112.88 42L-113.82 41.99L-114.04 41.99L-114.47 42L-114.6 41.99L-114.81 42L-115.03 42L-115.32 42L-116.37 42L-117.03 42L-117.03 43.81L-117.02 43.86L-116.98 43.87L-116.98 43.92L-116.94 43.98L-116.93 44.02L-116.97 44.05L-116.98 44.09L-116.93 44.1L-116.9 44.15L-116.9 44.18L-116.97 44.19L-116.97 44.24L-117.05 44.23L-117.1 44.28L-117.17 44.26L-117.22 44.29L-117.19 44.34L-117.24 44.4L-117.21 44.43L-117.22 44.48L-117.16 44.52L-117.12 44.58L-117.1 44.67L-117.05 44.74L-116.93 44.79L-116.93 44.81L-116.87 44.87L-116.83 44.93L-116.86 44.98L-116.85 45.02L-116.8 45.06L-116.78 45.11L-116.73 45.14L-116.7 45.24L-116.67 45.28L-116.68 45.31L-116.59 45.44L-116.55 45.46L-116.55 45.51L-116.46 45.6L-116.49 45.65L-116.54 45.69L-116.54 45.74L-116.59 45.78L-116.66 45.78L-116.7 45.82L-116.76 45.82L-116.82 45.88L-116.86 45.9L-116.92 46Z' },
...
function renderBuilderMarketHero(theme = {}, activeState = {}, selectedMarket = null, activeBuilders = []) {
  ...
  return `<section class="builder-market-hero market-world-${h(theme.accent)}" data-builder-market-world="${h(theme.stateCode)}" aria-label="${h(theme.label)} builder market world">
    ...
    <div class="builder-market-hero-art" aria-hidden="true">${renderBuilderStateSilhouette(theme.stateCode) || `<span>${h(theme.icon)}</span>`}<i></i><b>${h(theme.stateCode)}</b></div>
  </section>`;
}
...
function taxDeedLotAcres(row = {}) {
  const direct = Number(row.acreage || row.acres || row.lotSizeAcres);
  if (Number.isFinite(direct)) return direct;
  const match = String(row.lotSize || row.propertyUse || row.notes || '').match(/([0-9]+(?:\.[0-9]+)?)\s*ac\b/i);
...
function renderTaxDeedCsvDownloads() {
  const visibleDownloads = taxDeedCsvDownloads.filter(item => activeTaxDeedMarket === 'all' || String(item.market || '').includes(activeTaxDeedMarket));
  const laneCounts = visibleDownloads.reduce((acc, item) => {
    acc[item.lane] = (acc[item.lane] || 0) + 1;
    return acc;
  }, {});
  const totalRows = visibleDownloads.reduce((sum, item) => sum + Number(item.rows || 0), 0);
  const cards = visibleDownloads.map(item => `<article class="tax-deed-csv-card" data-tax-deed-csv-file="${h(item.file)}" data-tax-deed-csv-lane="${h(item.lane)}">
    <div class="tax-deed-csv-card-top"><span>${h(item.market)}</span><b>${h(item.title)}</b><em>${h(item.status)}</em></div>
    <dl>
      <div><dt>Rows</dt><dd>${h(item.rows)}</dd></div>
      <div><dt>Lane</dt><dd>${h(item.lane)}</dd></div>
    </dl>
    <div class="tax-deed-csv-actions">
      ${downloadLink(item.url, 'Download CSV', 'tax-deed-csv-download primary')}
      <a class="tax-deed-csv-download" href="${h(item.url)}" target="_blank" rel="noopener">Open</a>
    </div>
  ...
  return `<section class="tax-deed-csv-downloads phase310-tax-deed-csv-downloads" data-phase310-tax-deed-csv-downloads="latest-nonduplicate-static-csv-links" aria-label="Tax deed CSV downloads">
    <div class="tax-deed-csv-head">
      ...
      <div class="tax-deed-csv-metrics">
        ...
        <div><b>${h(laneCounts.buyer || 0)}</b><span>buyer files</span></div>
        <div><b>${h(laneCounts.owner || 0)}</b><span>owner files</span></div>
      </div>
    </div>
    <div class="tax-deed-csv-grid">${cards || '<article class="tax-deed-csv-card"><b>No CSV downloads match this filter.</b><p>Switch market to Every Tax-Deed market.</p></article>'}</div>
  </section>`;
}
...
function renderTaxDeedOwnerRunwayTable(runwayRows = []) {
  ...
  const rows = runway.rows.map((row, index) => {
    ...
    const proofLinks = [
      row.sourceUrl ? safeLink(row.sourceUrl, auctionLinkLabel, 'agent-source-link') : '',
  ...
  return `<div class="tax-deed-owner-loaded phase294-pa-upset-sale-owner-runway phase297-okaloosa-tax-deed-owner-runway phase298-york-2acre-owner-runway" data-phase294-pa-upset-sale-owner-runway="lot-size-skiptrace-risk-script" data-phase297-okaloosa-tax-deed-owner-runway="official-okaloosa-owner-lots-displayed" data-phase298-york-2acre-owner-runway="pa-2acre-leads-visible-default-owners-tab">
...
function renderTaxDeedBuyerPanel() {
  ...
  const allOwnerRows = buildTaxDeedOwnerRunway([
    ...
    ...asArray(okaloosaCountyTaxDeedOwnerRunway),
  ...
  const rows = buyers.map((buyer, index) => {
    ...
    const contactLinkLabel = contactUrl.includes('sunbiz') ? 'Sunbiz' : (contactUrl.includes('DocumentCenter') || contactUrl.includes('civicplus') ? 'Permit' : (contactUrl.includes('sellstate') || buyer.contactRole ? 'Profile' : 'Search'));
  ...
  target.innerHTML = `<section class="agent-referral-board buyer-validation-board tax-deed-redesign-board phase283-tax-deed-buyer-page phase293-tax-deed-page-tabs phase295-tax-deed-mission-control phase297-okaloosa-tax-deed-owner-runway phase298-york-2acre-owner-runway phase299-tax-deed-payment-visibility phase309-tax-deed-ia-clarity phase310-tax-deed-csv-downloads" data-phase309-tax-deed-ia="lane-first-market-matrix" aria-label="Tax deed operating tracker">
