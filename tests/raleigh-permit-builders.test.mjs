...
const kenHarvey = buyerOsintSignals.find(row => row.name === 'Ken Harvey Homes');
assert.ok(kenHarvey.buyBoxDetails?.propWireFilters?.length >= 3, 'Ken Harvey buy box should include tiered PropWire filter presets');
assert.match(JSON.stringify(kenHarvey.buyBoxDetails.propWireFilters), /Raleigh premium infill|0\.25-0\.75 ac|absentee|long-held|high equity/i, 'Ken Harvey PropWire filters should include infill seller-screening criteria');
