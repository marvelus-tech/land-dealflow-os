...
assert.match(stylesSource, /phase301-buybox-contrast-hierarchy/, 'PropWire filter cards must use the high-contrast hierarchy lock');
assert.match(stylesSource, /--propwire-label: #154b32/, 'PropWire label color must be dark enough on white, not neon lime');
assert.match(stylesSource, /\.buybox-propwire-card dt[\s\S]{0,220}color: var\(--propwire-label\)/, 'PropWire field labels must use accessible high-contrast label color');
