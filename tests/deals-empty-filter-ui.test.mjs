...
assert.match(app, /const texasLandReports = \[/, 'Land state reports must store report metadata in the project.');
...
assert.match(app, /Central Florida luxury lot \/ waterfront JV report/, 'FL Land reports must include the Central Florida luxury lot seller research report.');
assert.match(app, /function renderStateLandReports/, 'Selected state route must render the state report dock.');
assert.match(app, /phase261-land-report-dock/, 'State reports must have a dedicated report button/panel surface.');
assert.match(app, /Download full markdown/, 'State report panel must expose the full markdown report file.');
assert.match(app, /Blanket vacant-lot SMS/, 'TX report panel must include the vacant-lot SMS appetite-check scripts.');
assert.match(app, /Luxury-lot SMS scripts/, 'FL report panel must include luxury-lot SMS scripts.');
assert.match(app, /Hi \[Name\], this is Okeito\. I am reaching out privately about your property at\/near \[Address\]/, 'FL report panel must expose the recommended luxury-lot SMS.');
