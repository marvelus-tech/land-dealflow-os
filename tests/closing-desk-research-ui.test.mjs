...
assert.match(appSource, /Reference layer only\. The active workflow stays above: control, assign, close\./, 'Closing Desk research should remain available as a secondary reference layer');
...
assert.match(appSource, /Legal packet workflow/, 'Closing route should frame the contract tools as a legal packet workflow');
assert.match(appSource, /Control\. Assign\. Close\./, 'Closing route should present the three-step packet path');
