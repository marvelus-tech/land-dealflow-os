...
assert.match(appSource, /Suggested order, not a lock/, 'Builders header should frame market order as selectable guidance, not a forced sequence');
...
assert.match(appSource, /market-toggle-v34[\s\S]{0,280}<small class="market-row-main"><kbd class="market-code">/, 'market card should use no-span row wrappers for stable market metrics');
assert.match(appSource, /market-builder-total[\s\S]{0,260}<b>\$\{h\(market\.builderCount\)\}<\/b><small>/, 'market card should separate builder numeral from its metric label');
assert.match(appSource, /market-contact-copy[\s\S]{0,220}\$\{h\(market\.statusCopy\)\}/, 'market card should expose live/thin/needs-source status copy');
assert.match(appSource, /class="market-contact-rail" role="progressbar"[\s\S]{0,420}market\.minimumUniqueBuilders/, 'market card should include a semantic source-completion rail');
assert.match(appSource, /minimum builders collected for \$\{market\.label\}/, 'market rail should carry accessible minimum-builder progress copy');
